...
void ICM42688_Data::into_message(uint8_t *msg_data) {
	for (int i = 0; i < 3; i++) {
		...
		msg_data[0 + 2*i] = (uint8_t)(acc_data >> 8);
		msg_data[1 + 2*i] = (uint8_t)(acc_data & 0xFF);
...
ICM42688_Data ICM42688::get_avg_data(int num_avgs) {
	ICM42688_Data data = { 0 };

	for (int i = 0; i < num_avgs; i++) {
		ICM42688_Data single_data = get_data();
//		osDelay(2);
		int invalid_data = 0;

		for (int j = 0; j < 3; j++) {
			data.acc[j] += single_data.acc[j];
			data.gyro[j] += single_data.gyro[j];
		}
		data.temp += single_data.temp;
	...
	for (int i = 0; i < 3; i++) {
		data.acc[i] /= (float)num_avgs;
		data.gyro[i] /= (float)num_avgs;
	}
	data.temp /= (float)num_avgs;

	return data;  // Success
}


ICM42688_Data ICM42688::get_data(void) {
	ICM42688_Data data = { 0 };
...
void ICM42688_FIFO::getFifoAccelX_mss(size_t* size, float* data) {
	*size = _aSize;
	std::memcpy(data, _axFifo, _aSize * sizeof(float));
}
...
void ICM42688_FIFO::getFifoAccelY_mss(size_t* size, float* data) {
	*size = _aSize;
	std::memcpy(data, _ayFifo, _aSize * sizeof(float));
}
